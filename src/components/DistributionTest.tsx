          ...
          <div className="distribution-controls">
            <label>
              ...
              <input
                ...
                onChange={(e) =>
                  setBallCount(
                    Math.max(2, Math.min(5000, parseInt(e.target.value) || 100))
                  )
                }
                disabled={isRunning}
              />
            ...
            <div className="distribution-results">
              ...
              <div className="distribution-chart">
                {testResults.map((count, index) => {
                  const percentage =
                    totalBalls > 0 ? (count / totalBalls) * 100 : 0;
                    ...
                    <div key={index} className="chart-bar-container">
                      ...
                      <div className="chart-label">
                        <span
                          ...
                          title={slotNames[index] || `Slot ${index + 1}`}
                        >
              ...
              <div className="distribution-stats">
                <p>
                  <strong>Expected per slot:</strong>{" "}
                  {(100 / numSlots).toFixed(1)}%
                </p>
                <p>
                  <strong>Standard deviation:</strong>{" "}
                  {calculateStdDev(testResults, totalBalls / numSlots).toFixed(
                    2
                  )}
                </p>
              </div>
...
function calculateStdDev(counts: number[], expected: number): number {
  ...
  const avgSquaredDiff =
    squaredDiffs.reduce((a, b) => a + b, 0) / counts.length;
